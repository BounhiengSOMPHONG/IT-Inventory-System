<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Product Types') }}</h2>
    </x-slot>
    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="mb-6">
                ...
                <p class="text-gray-600 mt-1">Manage product types</p>
            </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <div>
                        <h3 class="text-lg font-medium text-gray-900">Product Type List</h3>
                        <p class="text-sm text-gray-500 mt-1">All product types in the system</p>
                    </div>
                ...
                <div class="p-6">
                    ...
                    <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <form method="GET" action="{{ route('product-types.index') }}" class="flex items-center w-full sm:w-auto">
                            <label for="search" class="sr-only">Search product types</label>
                            <div class="relative w-full sm:w-72">
                                <input id="search" name="search" value="{{ old('search', $search ?? '') }}" type="text" placeholder="Search product types..." class="block w-full pl-10 pr-3 py-2 border border-gray-200 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" />
                            ...
                            @if(!empty($search))
                                <a href="{{ route('product-types.index') }}" class="ml-2 inline-flex items-center px-3 py-2 bg-gray-100 text-gray-700 rounded-md text-sm">Clear</a>
                            @endif
                        </form>

                        <div class="text-sm text-gray-600">
                            <span class="font-medium">Showing</span>
                            <span class="ml-1">{{ $productTypes->firstItem() ?? 0 }}-{{ $productTypes->lastItem() ?? 0 }}</span>
                    ...
                    @else
                        <div class="text-center py-12">
                            <h3 class="mt-2 text-sm font-medium text-gray-900">No product types</h3>
                            <p class="mt-1 text-sm text-gray-500">Get started by creating a new product type.</p>
                            <div class="mt-6">
                                <a href="{{ route('product-types.create') }}" 
                                   class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 focus:bg-indigo-700 active:bg-indigo-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                                    Create Product Type

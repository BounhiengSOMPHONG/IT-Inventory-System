...
const ProductTypeController = {
    ...
    async list(req, res) {
        const pts = await ProductTypeService.list();
        res.json(pts);
    },
    async get(req, res) {
    ...
    , async remove(req, res) {
        const id = req.params.id;
        const detele = await ProductTypeService.remove(id);
        if (!detele) return res.status(404).json({ message: 'Product Type not found' });
